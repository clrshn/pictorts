@php
    ...
    $formMode = $formMode ?? ($document ? 'edit' : 'create');
    $isEdit = $formMode === 'edit';
    $isModal = $isModal ?? false;
    $isTravelOrder = $isTravelOrder ?? (($document?->document_type ?? null) === 'TO');
    $formAction = $formAction ?? ($isEdit ? route('documents.update', $document) : route('documents.store'));
    $cancelUrl = $cancelUrl ?? route('documents.index', $isTravelOrder ? ['type' => 'TO'] : []);
    $returnUrl = $returnUrl ?? ($isModal
        ? request()->fullUrl()
        : ($isEdit ? route('documents.show', $document) : route('documents.index', $isTravelOrder ? ['type' => 'TO'] : [])));
    $formKey = $formKey ?? ($isEdit ? 'document-edit-' . $document->id : 'document-create');
@endphp
...
<div id="{{ $formKey }}-container" data-document-form-root style="padding:24px;">
    <form method="POST" action="{{ $formAction }}" enctype="multipart/form-data">
        ...
        <input type="hidden" name="modal_record_id" value="{{ $document?->id }}">
        ...
        <div style="display:grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap:16px;">
            <div class="form-group">
                ...
                <select name="document_type" data-role="document-type" class="form-control" required>
                    <option value="">Select Type</option>
                    @foreach(['MEMO','EO','SO','LETTER','SP','TO','OTHERS'] as $typeOption)
                        <option value="{{ $typeOption }}" {{ old('document_type', $document?->document_type ?? ($isTravelOrder ? 'TO' : null)) === $typeOption ? 'selected' : '' }}>{{ $typeOption === 'TO' ? 'TO - Travel Order' : $typeOption }}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group">
                <label>Communication Type <span style="color:#c0392b">*</span></label>
                <select name="direction" data-role="direction" class="form-control" required>
                    <option value="">Select</option>
                    <option value="INCOMING" {{ old('direction', $document?->direction) === 'INCOMING' ? 'selected' : '' }}>INCOMING</option>
                    <option value="OUTGOING" {{ old('direction', $document?->direction ?? ($isTravelOrder ? 'OUTGOING' : null)) === 'OUTGOING' ? 'selected' : '' }}>OUTGOING</option>
                </select>
            </div>
        </div>

        <div style="display:grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap:16px;">
            <div class="form-group" data-role="delivery-scope-group" style="{{ old('direction', $document?->direction ?? ($isTravelOrder ? 'OUTGOING' : null)) === 'OUTGOING' ? '' : 'display:none;' }}">
                <label>Outgoing Type <span style="color:#c0392b">*</span></label>
                <select name="delivery_scope" data-role="delivery-scope" class="form-control" {{ old('direction', $document?->direction ?? ($isTravelOrder ? 'OUTGOING' : null)) === 'OUTGOING' ? 'required' : '' }}>
                    <option value="">Select Type</option>
                    <option value="EXTERNAL" {{ old('delivery_scope', $document?->delivery_scope) === 'EXTERNAL' ? 'selected' : '' }}>External</option>
                    <option value="INTERNAL" {{ old('delivery_scope', $document?->delivery_scope ?? ($isTravelOrder ? 'INTERNAL' : null)) === 'INTERNAL' ? 'selected' : '' }}>Internal</option>
                </select>
            ...
            <div class="form-group">
                <label>Originating Office <span data-role="originating-required" style="color:#c0392b;">*</span></label>
                <select name="originating_office" data-role="originating-office" class="form-control" {{ old('direction', $document?->direction) === 'OUTGOING' ? '' : 'required' }}>
        ...
        <div data-role="travel-order-fields" style="{{ $isTravelOrder ? '' : 'display:none;' }}">
            <div style="display:grid; grid-template-columns: repeat(3, 1fr); gap:16px;">
                <div class="form-group">
                    <label>Travel Order Type <span style="color:#c0392b">*</span></label>
                    <select name="travel_order_type" data-role="travel-order-type" class="form-control">
                        <option value="">Select Type</option>
        ...
        <div class="form-group" data-role="subject-group" style="{{ $isTravelOrder ? 'display:none;' : '' }}">
            <label>Subject / Title <span style="color:#c0392b">*</span></label>
            <input type="text" name="subject" data-role="subject-input" class="form-control" value="{{ old('subject', $document?->subject) }}" {{ $isTravelOrder ? '' : 'required' }}>
        </div>
        <input type="hidden" name="subject" data-role="travel-subject" value="{{ old('subject', $document?->subject ?: 'Travel Order') }}" {{ $isTravelOrder ? '' : 'disabled' }}>

        <div data-role="letter-fields" style="{{ old('document_type', $document?->document_type) === 'LETTER' ? '' : 'display:none;' }}">
            <div style="display:grid; grid-template-columns: repeat(3, 1fr); gap:16px;">
                <div class="form-group">
        ...
        <div style="display:grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap:16px;">
            <div class="form-group">
                ...
                <input type="date" name="date_received" class="form-control" value="{{ old('date_received', $document?->date_received?->format('Y-m-d')) }}" required>
                <small style="color:#999;">{{ $isEdit ? 'Changing the date will regenerate Tracking Code and Transaction Number to match the new year.' : 'Used for tracking number generation (year)' }}</small>
            </div>
            <div class="form-group">
            ...
            <div style="display:grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap:16px;">
                <div class="form-group">
                    ...
                    <select name="status" class="form-control">
                        @foreach(['ONGOING','DELIVERED','DONE'] as $statusOption)
                            <option value="{{ $statusOption }}" {{ old('status', $document?->status) === $statusOption ? 'selected' : '' }}>{{ $statusOption }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group">
        ...
        <div style="margin-top:8px; padding:16px; border-radius:14px; border:1px solid rgba(148,163,184,0.22); background:linear-gradient(135deg,#ffffff 0%,#f8fafc 100%);">
            ...
            <div style="display:grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap:16px;">
                ...
                <div class="form-group" style="margin:0;">
                    <label>{{ $isEdit ? 'Add More Files' : 'Attach Files' }} (PDF, Docs, Sheets, Images)</label>
        ...
        <div style="display:flex; gap:10px; margin-top:8px;">
            ...
            <button type="submit" class="btn-red"><i class="fas fa-save"></i> {{ $isEdit ? ($isTravelOrder ? 'Update Travel Order' : 'Update Document') : ($isTravelOrder ? 'Save Travel Order' : 'Save Document') }}</button>
            @if(!$isModal)
                @if($isEdit)
...
<script>
(function() {
    const root = document.getElementById(@json($formKey . '-container'));
    if (!root) {
        return;
    }

    const directionSelect = root.querySelector('[data-role="direction"]');
    const originatingOfficeSelect = root.querySelector('[data-role="originating-office"]');
    const requiredSpan = root.querySelector('[data-role="originating-required"]');
    const documentTypeSelect = root.querySelector('[data-role="document-type"]');
    const letterFields = root.querySelector('[data-role="letter-fields"]');
    const deliveryScopeGroup = root.querySelector('[data-role="delivery-scope-group"]');
    const deliveryScopeSelect = root.querySelector('[data-role="delivery-scope"]');
    const travelOrderFields = root.querySelector('[data-role="travel-order-fields"]');
    const travelOrderTypeSelect = root.querySelector('[data-role="travel-order-type"]');
    const subjectGroup = root.querySelector('[data-role="subject-group"]');
    const subjectInput = root.querySelector('[data-role="subject-input"]');
    const hiddenTravelSubject = root.querySelector('[data-role="travel-subject"]');
    const particularsLabel = root.querySelector('[data-role="particulars-label"]');

    if (!directionSelect || !originatingOfficeSelect || !documentTypeSelect || !deliveryScopeGroup || !deliveryScopeSelect || !travelOrderFields || !travelOrderTypeSelect || !subjectGroup || !subjectInput || !hiddenTravelSubject || !particularsLabel || !letterFields) {
        return;
    }
    ...
    function toggleLetterFields() {
        letterFields.style.display = documentTypeSelect.value === 'LETTER' ? 'block' : 'none';
    }
