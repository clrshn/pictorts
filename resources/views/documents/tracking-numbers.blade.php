<x-app-layout>
    ...
    <div class="table-card">
        <div style="background:#333; color:#fff; padding:10px 20px; font-weight:600; font-size:13px; display:flex; justify-content:space-between; align-items:center;">
            ...
            <div>
                <a href="{{ route('documents.create') }}" class="btn-green">
                    <i class="fas fa-plus"></i> Create Document
                </a>
            </div>
        </div>
        <div style="padding:20px;">
            <div style="background:#f8f9fa; border:1px solid #e9ecef; border-radius:4px; padding:12px 16px; margin-bottom:20px;">
                <strong style="color:#333;">Format:</strong> PICTO-{TYPE}-{YEAR}-{SEQUENCE}<br>
                <small style="color:#666;">Each document type has its own numbering sequence that resets yearly.</small>
            </div>
                    ...
                    <div style="margin-bottom:32px;">
                        ...
                        @foreach($years as $year => $documents)
                            <div style="margin-bottom:20px;">
                                <h4 style="color:#555; margin-bottom:12px; font-size:14px;">
                                    Year {{ $year }} ({{ count($documents) }} documents)
                                </h4>
                                
                                <div style="display:grid; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); gap:12px;">
                                    @foreach($documents as $doc)
                                        <div style="border:1px solid #ddd; border-radius:4px; padding:12px; background:#fff;">
