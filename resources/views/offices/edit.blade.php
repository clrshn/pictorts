<x-app-layout>
    <x-slot name="header">
        <h1>Edit Office</h1>
        <div class="breadcrumb"><a href="{{ route('dashboard') }}">Home</a> / <a href="{{ route('offices.index') }}">Office Management</a> / Edit Office</div>
    </x-slot>
    ...
    <div class="filter-box office-search-box">
        <div style="display:flex; justify-content:space-between; align-items:center; gap:16px; margin-bottom:16px; flex-wrap:wrap;">
            <h3 style="margin:0;">Check Existing Offices</h3>
            <span class="office-search-count">{{ $offices->count() }} other office{{ $offices->count() === 1 ? '' : 's' }}</span>
        </div>

        <div class="office-search-field">
            <input type="text" id="officeLookup" class="form-control" placeholder="Search office code or name to avoid duplicates...">
        </div>
        ...
        <div class="office-search-results" id="officeLookupResults">
            ...
            @empty
                <div class="office-search-empty">No other offices found.</div>
            @endforelse
        </div>
    </div>
    ...
    <div class="table-card">
        <div class="table-header">
            <h3>Edit Office: {{ $office->code }}</h3>
        </div>

        <form method="POST" action="{{ route('offices.update', $office) }}" style="padding:20px;">
            @csrf @method('PUT')
            
            <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(300px,1fr)); gap:20px;">
                <div class="form-group">
                    <label for="code">Office Code *</label>
                    <input type="text" id="code" name="code" class="form-control" value="{{ old('code', $office->code) }}" required maxlength="10" placeholder="e.g., OPG" style="width:100%; padding:10px; border:1px solid #ddd; border-radius:4px; font-size:14px;">
                    @error('code')
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="name" name="name" class="form-control" value="{{ old('name', $office->name) }}" required placeholder="e.g., Office of the Provincial Governor" style="width:100%; padding:10px; border:1px solid #ddd; border-radius:4px; font-size:14px;">
                    @error('name')
                        <div style="color:#e74c3c; font-size:12px; margin-top:4px;">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div style="margin-top:30px; display:flex; gap:10px; justify-content:flex-end;">
                ...
                <button type="submit" class="btn-red"><i class="fas fa-save"></i> Update Office</button>
            </div>
        </form>
    </div>
